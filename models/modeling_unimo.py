...
class CLIPAttention(nn.Module):
    ...
    def forward(
        ...
        if past_key_values is not None:
            # print("%"*20)
            # print(past_key_values[0].shape, key_states.shape)
            # print(past_key_values[1].shape, value_states.shape)
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if past_key_values is not None:
            ...
            # print('past_key_values',type(past_key_values),len(past_key_values),past_key_values[0].shape,past_key_values[1].shape)
            key_layer=torch.cat([key_layer,past_key_values[0]],dim=2)
            value_layer=torch.cat([value_layer,past_key_values[1]],dim=2)
        ...
        # print("000",attention_scores.shape,attention_mask.shape)
        # 如果有past_key_values，就不attention mask了
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        # print("outputs.shape",len(outputs),outputs[0].shape,outputs[1].shape)
...
class UnimoEncoder(nn.Module):
    def __init__(self, vision_config, text_config):
        ...
        self.struct_layer = nn.ModuleList([BertLayer(text_config) for _ in range(text_config.num_hidden_layers)])
    ...
    def forward(
        ...
        struct_hidden_states = struct_embeds
        ...
        for idx in range(self.vision_config.num_hidden_layers):
            ...
            # past_key_values = struct_layer_output[-1] if idx >= 8 else None
            past_key_values = (vision_hidden_states.view([bsz,head_nums,-1,dim]),vision_hidden_states.view([bsz,head_nums,-1,dim])) if idx >= 8 else None
            # past_key_values = None
            ...
            #struct
            last_hidden_state = text_hidden_states if idx >= 8 else None
            # last_hidden_state = None
            # past_key_values = text_layer_output[-1] if idx >= 8 else None
            past_key_values = (vision_hidden_states.view([bsz,head_nums,-1,dim]),vision_hidden_states.view([bsz,head_nums,-1,dim])) if idx >= 8 else None
            # past_key_values = None
            ...
            struct_layer_module = self.struct_layer[idx]
            struct_layer_output = struct_layer_module(
            ...
            struct_hidden_states = struct_layer_output[0]

            # print("vision_hidden_states.shape",vision_hidden_states.shape)
            # print("text_layer_output[-1].shape",text_layer_output[-1][0].shape)
...
class UnimoModel(nn.Module):
    def __init__(self, vision_config, text_config, add_pooling_layer=True):
        ...
        self.vision_pooler= VisionPooler(vision_config)

        # text model+ struct
        self.text_config = text_config
        self.text_embeddings = BertEmbeddings(text_config)
        self.text_pooler = BertPooler(text_config) if add_pooling_layer else None
        # self.struct_pooler = BertPooler(text_config) if add_pooling_layer else None
    ...
    def forward(
        ...
        # print('vision_embedding_output',vision_embedding_output.shape)

        # pre text
        input_shape = t_input_ids.size()
        batch_size, seq_length = input_shape
        device = t_input_ids.device
        if t_attention_mask is None:
            t_attention_mask = torch.ones(((batch_size, seq_length)), device=device)
        if t_token_type_ids is None:
            if hasattr(self.text_embeddings, "token_type_ids"):
                ...
                t_token_type_ids = buffered_token_type_ids_expanded
            else:
                t_token_type_ids = torch.zeros(input_shape, dtype=torch.long, device=device)

        t_extended_attention_mask: torch.Tensor = get_extended_attention_mask(t_attention_mask, input_shape, device)
        t_head_mask = get_head_mask(t_head_mask, self.text_config.num_hidden_layers)    # [None]*12
        ...
        # pre struct
        input_shape = s_input_ids.size()
        batch_size, seq_length = input_shape
        device = s_input_ids.device
        if s_attention_mask is None:
            s_attention_mask = torch.ones(((batch_size, seq_length)), device=device)
        if s_token_type_ids is None:
            if hasattr(self.text_embeddings, "token_type_ids"):
                buffered_token_type_ids = self.text_embeddings.token_type_ids[:, :seq_length]
                buffered_token_type_ids_expanded = buffered_token_type_ids.expand(batch_size, seq_length)
                s_token_type_ids = buffered_token_type_ids_expanded
            else:
                s_token_type_ids = torch.zeros(input_shape, dtype=torch.long, device=device)

        s_extended_attention_mask: torch.Tensor = get_extended_attention_mask(s_attention_mask, input_shape, device)
        s_head_mask = get_head_mask(s_head_mask, self.text_config.num_hidden_layers)    # [None]*12

        struct_embedding_output = self.text_embeddings(
        ...
        encoder_outputs = self.encoder(
            ...
            t_attention_mask=t_extended_attention_mask,

            struct_embeds=struct_embedding_output,
            s_attention_mask=s_extended_attention_mask,
        ...
        sequence_output,vision_output,struct_output= encoder_outputs[0],encoder_outputs[1],encoder_outputs[2]
        ...
        v_pooled_output=self.vision_pooler(vision_output) if self.vision_pooler is not None else None

        # print('v_pooled_output',v_pooled_output.shape)
        return_dict=False
        # print('return_dict111:',return_dict)
        if not return_dict:
            # print('return_dict112:',return_dict)
            return (sequence_output, pooled_output,v_pooled_output,struct_output) 
...
class UnimoForMaskedLM(nn.Module):
    def __init__(self, vision_config, text_config):
        ...
        self.cls = UnimoOnlyMLMHead(text_config,vision_config)
    ...
    def forward(
        ...
        return_dict=False
        # print('return_dict44',return_dict)
        outputs = self.unimo(
        ...
        vision_output=outputs[2]
        struct_output=outputs[3]
        # print('return_dict11',return_dict)
        # print(sequence_output.shape,vision_output.shape,struct_output.shape)
        # 使用三流信息共同打分
        prediction_scores = self.cls(sequence_output,vision_output,struct_output,pretrain=pretrain,struct_poss=struct_poss)
...
class UnimoOnlyMLMHead(nn.Module):
    def __init__(self, config,config1):
        ...
        self.predictions_v= UnimoLMPredictionHeadVision(config,config1)
        self.predictions_t= UnimoLMPredictionHeadText(config,config)

    def forward(self, sequence_output,vision_output,struct_output,pretrain=True,struct_poss=1):

        # 结构信息打分
        prediction_scores = self.predictions(struct_output)
        # 文本信息打分
        pv,prediction_scores_v=self.predictions_v(vision_output,struct_output,pretrain=pretrain)
        # 视觉信息打分
        pt,prediction_scores_t=self.predictions_t(sequence_output,struct_output,pretrain=pretrain)

        prediction_scores_v=prediction_scores_v.unsqueeze(1)
        prediction_scores_t=prediction_scores_t.unsqueeze(1)

        # prediction_scores=torch.sigmoid(prediction_scores)
        # prediction_scores_v=torch.sigmoid(prediction_scores_v)
        # prediction_scores_t=torch.sigmoid(prediction_scores_t)
                
        # prediction_scores=torch.nn.functional.relu(prediction_scores)
        # prediction_scores_v=torch.nn.functional.relu(prediction_scores_v)
        # prediction_scores_t=torch.nn.functional.relu(prediction_scores_t)

        # prediction_scores=F.softmax(prediction_scores,dim=-1)
        # prediction_scores_v=F.softmax(prediction_scores_v,dim=-1)
        # prediction_scores_t=F.softmax(prediction_scores_t,dim=-1)
        
        # return prediction_scores 
        # return prediction_scores + pv.unsqueeze(1) * prediction_scores_v.unsqueeze(1) 
        # return prediction_scores + pt.unsqueeze(1) * prediction_scores_t.unsqueeze(1)

        print("pppppppp",struct_poss,pv[0],pt[0])
        ...
        # return prediction_scores
        # S+V
        # return 0.8*prediction_scores + 0.2 * prediction_scores_v 
        # S+T
        # return 0.7*prediction_scores + 0.3 * prediction_scores_t
        # S+T+V
        return c * prediction_scores + b * prediction_scores_v + a * prediction_scores_t
        # return struct_poss * prediction_scores + pv.unsqueeze(1) * prediction_scores_v + pt.unsqueeze(1) * prediction_scores_t
        



class UnimoLMPredictionHeadVision(nn.Module):
    def __init__(self,config,config1):
        super().__init__()

        self.p_decoder=nn.Linear(config.hidden_size+config1.hidden_size, 1, bias=False)
        self.p_bias = nn.Parameter(torch.zeros(1))

        self.p_decoder.bias=self.p_bias
        self.decoder = nn.Linear(config.hidden_size+config1.hidden_size, 46386, bias=False)
        self.bias = nn.Parameter(torch.zeros(46386))
    ...
    def forward(self,vision_output,struct_output,pretrain=True):
        # print("--1---",vision_output.shape,struct_output.shape)
        hv_cls=vision_output
        # query
        if pretrain:
            print("pretrain!")
            h_query=struct_output[:,0,:].squeeze()
        else:
            print("finetune!")
            # h_query=struct_output[:,3,:].squeeze()
            h_query=struct_output[:,0,:].squeeze()
            # h_query=torch.mean(struct_output[:,4:6,:].squeeze(),dim=1)

        hidden_state=torch.cat((hv_cls,h_query),-1)
        # hidden_state=self.transform(hidden_state)
        pv=self.p_decoder(hidden_state)
        pv=torch.sigmoid(pv)
        # pv=torch.nn.functional.relu(pv)
        prediction_scores_v=self.decoder(torch.cat((hv_cls,h_query),-1))

        return pv,prediction_scores_v

class UnimoLMPredictionHeadText(nn.Module):
    def __init__(self,config,config1):
        super().__init__()
    ...
    def forward(self,text_output,struct_output,pretrain=True):
        # print("--2---",text_output.shape,struct_output.shape)

        ht_cls=text_output[:,0,:].squeeze()
        # query向量
        if pretrain:
            print("pretrain!")
            h_query=struct_output[:,0,:].squeeze()
        ...
        hidden_state=torch.cat((ht_cls,h_query),-1)
        # hidden_state=self.transform(hidden_state)
        pt=self.p_decoder(hidden_state)
        pt=torch.sigmoid(pt)
        # pt=torch.nn.functional.relu(pt)
        prediction_scores_t=self.decoder(torch.cat((ht_cls,h_query),-1))

        return pt,prediction_scores_t
...
class UnimoLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        # print('config.vocab_size1',config.vocab_size)
